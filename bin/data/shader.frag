...
void main()
{
    ...
    if(shaderMode == 1){
        vec4 weightsColor = texture2D(weightsM,TexCoord);
        float weightsValue = map(unpack(weightsColor));

        vec4 inputColor = texture2D(inputV,vec2(TexCoord.x,0.0));
    ...
    if(shaderMode == 2){
        if(gl_FragCoord.x <= 1.0){
            ...
            for(float i = 1.0/float(InputSize*2); i <= 1.0; i+= 1.0/float(InputSize)){
                sumPixels += map(unpack(texture2D(weightsM,vec2(i,TexCoord.y))));
            }
    ...
    if(shaderMode == 3){
        ...
        if(gl_FragCoord.y < 1.0){
            ...
            for(float i = 1.0/float(OutputSize*2); i <= 1.0; i += 1.0/float(OutputSize)){
                sumError += map(unpack(texture2D(weightsM,vec2(TexCoord.x,i)))) * map(unpack(texture2D(errorV,vec2(0.0,i))));
            }
            float inputOfNeuron = unpack(texture2D(inputV,vec2(TexCoord.x,0.0)));
    ...
    //calculate Momentum
    if(shaderMode == 4){
        vec4 weightsColor = texture2D(weightsM,TexCoord.xy);
        float weightsValue = map(unpack(weightsColor));
        //weightsValue = weightsValue * (1.0 - abs(weightsValue*weightsValue*weightsValue*weightsValue*weightsValue*weightsValue));

        vec4 errorColor = texture2D(errorV,vec2(0.0,TexCoord.y));
        float errorValue = map(unpack(errorColor));

        vec4 inputColor = texture2D(inputV,vec2(TexCoord.x,0.0));
        float inputValue = unpack(inputColor);

        vec4 momentumColor = texture2D(momentumM,TexCoord.xy);
    ...
    if(shaderMode == -1){
        vec4 pixelColor = texture2D(weightsM,TexCoord);
