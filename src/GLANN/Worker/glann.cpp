...
vector<float> GLANN::propergateFW(vector<float> input, ANNData* netToProcess){
  ...
  for(int i = 0; i < netToProcess->mInput.getWidth(); i++)
        netToProcess->mInput.setColor(i,0,GLANNTools::pack(input[i]));
	
	//Bias
    netToProcess->mInput.setColor(netToProcess->mInput.getWidth()-1,0,GLANNTools::pack(0.999));
    ...
    vector<float> output;
    for(int i = 0; i < netToProcess->mOutput.getHeight(); i++)
...
vector<float> GLANN::propergateBW(vector<float> input, vector<float> error
  ...
  for(int i = 0; i < netToProcess->mInput.getWidth(); i++)
    ...
    for(int i = 0; i < netToProcess->mError.getHeight(); i++)
    ...
    error.clear();
    for(int i = 0; i < netToProcess->mInput.getWidth()-1; i++)
    for(int i = 0; i < netToProcess->mInput.getWidth()-1; i++)
